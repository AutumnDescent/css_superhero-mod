...
def weapon_give(userid,weapon,popupname):
    ...
    if int(player.isdead) != 1:
        bat_weapons = superhero.Users[userid]['bat_weapons']
        if weapon in bat_weapons:
            ...
            if popuplib.isqueued('bat_menu', userid):
                popuplib.close('bat_menu', userid)
            string = bat_weapons[0]
            for key in bat_weapons:
                if not key in string:
                    string = string+','+str(key)
            superhero.Users[userid]['bat_weapons'] = string
        else:
...
def player_spawn(ev):
    ...
    if superhero.hasHero(userid,'Batman'):
        ...
        if int(player.isdead) != 1:
            ...
            superhero.Users[userid]['bat_weapons'] = 'tmp,mp5navy,p90,ak47,m4a1,sg552'
            if popuplib.isqueued('bat_menu',userid):
                popuplib.close('bat_menu',userid)
            if not primary:
                popuplib.send('bat_menu',userid)